...
impl SeqPosBaseModProbs {
    ...
    pub fn set_skip_mode(&mut self, skip_mode: SkipMode) {
    ...
    pub fn remove_implicit_probs(self) -> Self {
        let probs = self
            .pos_to_base_mod_probs
            .into_iter()
...
pub fn format_mm_ml_tag<B: ParseChar>(
    positions_to_probs: SeqPosBaseModProbs,
    primary_base_symbol: B,
    cumulative_counts: &[u32],
    strand: Strand,
) -> (String, Vec<u8>) {
    ...
    if mod_code_to_position.is_empty() {
        mm_tag.push_str(&format!(
            "{}{}{}{};",
            primary_base_symbol.char(),
            strand.to_char(),
            primary_base_symbol.char(), /* "any mod" for a base is the same
                                         * char as itself */
            skip_mode_label
        ));
    } else {
